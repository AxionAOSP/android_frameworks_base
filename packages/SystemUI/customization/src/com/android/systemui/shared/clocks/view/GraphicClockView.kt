...
class GraphicClockView @JvmOverloads constructor(
    ...
    override fun drawClock(canvas: Canvas) {
        ...
        val clockSize = min(width, height).toFloat()
        val isLargeClockMode = height > width * 1.5f
        val yOffset = if (isLargeClockMode) -clockSize / 4f else 0f
    ...
    private fun drawHand(
        ...
        // Use the smaller dimension to keep hands proportional
        val baseSize = min(width, height).toFloat()
        val startLength = multipliers.first * baseSize
        val endLength = multipliers.second * baseSize
