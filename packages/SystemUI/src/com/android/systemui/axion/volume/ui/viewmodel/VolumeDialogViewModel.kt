...
class AxionVolumeDialogViewModel @Inject constructor(
    ...
    private val stableStreamOrder = mutableListOf<Int>()
    ...
    val sliderItems: StateFlow<List<VolumeSliderItem>> = uiState
        .map { state ->
            ...
            for (stream in dialogState.volumeStreams) {
                if (stream.streamType !in stableStreamOrder) {
                    val insertIdx = streamOrder.indexOf(stream.streamType)
                    if (insertIdx >= 0) {
                        val pos = stableStreamOrder.indexOfFirst { existing ->
                            val existingIdx = streamOrder.indexOf(existing)
                            existingIdx < 0 || existingIdx > insertIdx
                        }
                        if (pos >= 0) stableStreamOrder.add(pos, stream.streamType)
                        else stableStreamOrder.add(stream.streamType)
                    } else {
                        stableStreamOrder.add(stream.streamType)
                    }
                }
            }

            val orderedStreams = stableStreamOrder
                .mapNotNull { currentStreams[it] }
