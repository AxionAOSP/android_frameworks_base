...
private class BarHeightsState(barCount: Int) {
    ...
    fun updateTargets(newTargets: FloatArray) {
        if (targetHeights.size != newTargets.size) {
            currentHeights = FloatArray(newTargets.size) { i -> 
                if (i < currentHeights.size) currentHeights[i] else 2f 
            }
            targetHeights = FloatArray(newTargets.size)
        }
        for (i in newTargets.indices) {
            targetHeights[i] = newTargets[i]
        }
    }
    
    fun applySmoothing(smoothingFactor: Float) {
        for (i in currentHeights.indices) {
            val target = if (i < targetHeights.size) targetHeights[i] else 2f
            currentHeights[i] = currentHeights[i] + smoothingFactor * (target - currentHeights[i])
        }
    }
}
...
fun PulseVisualizer(
    ...
    LaunchedEffect(state.isVisible) {
        while (isActive && state.isVisible) {
            withFrameNanos { _ ->
                barHeights.applySmoothing(state.smoothingFactor)
                frameCount++
            }
        }
    }
