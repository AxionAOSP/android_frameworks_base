...
class LockScreenWidgets(context: Context, attrs: AttributeSet) : LinearLayout(context, attrs) {
    ...
    private val viewController: LockScreenWidgetsController = LockScreenWidgetsController(this)
    
    fun init() {
        viewController.registerCallbacks()
    }
    
    fun deInit() {
        viewController.unregisterCallbacks()
    }
